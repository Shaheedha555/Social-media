    ...
    const [otp,setOtp] = useState({
     ...
     async function handleSubmit(e) {
        ...
        if(result.payload.status === true){
            const {_doc:{name,email,contact},token} = result.payload
            console.log(name,email,contact,token);

            localStorage.setItem('user',JSON.stringify({name,email,contact,token}))
            navigate('/')}
          ...
          <div style={{ display: display }} id="myModal" className="modal">
              ...
              <div className="modal-content">
                  <div className="modal-header">
                      ...
                      <h2> {modal ? 'Select email or contact' : 'Verify your account'} </h2>
                  </div>
                  { modal && <FormBody error={error} errMsg={errMsg} onclick={()=>{setMethod('email')}} method={method} email={email} contact={contact} onButtonClick={onClick} onInputClick={()=>{setMethod('contact')}} />
                  }
                  
                  { !modal && <OtpBody handleSubmit={handleSubmit} otp={otp} handleChange={handleChange} inputfocus={inputfocus} resendOtp={onClick}/>
}
              </div>
...
//     <OTPmodal onclick={()=>{setModal(true)}} method={method} data={ method ==='email' ? email : contact}/>
 ...
 export const FormBody = (props) =>{
    const {error,errMsg,onclick,method,email,contact,onButtonClick,onInputClick} = props
    return(
        <form>
            <div className="modal-body">
                ...
                <div className="radio-grp">
                    ...
                    <input 
                     ...
                     value={method}
                    />
                    <label htmlfor='contact' ><span>Mobile</span>({contact})</label>
                </div>
            </div>

            <div className="modal-footer">
                <Button onclick={onButtonClick}  class="primarySmall">Send OTP</Button>
            </div>
        </form >
    )
}
...
<form onSubmit={handleSubmit}>
                      ...
                      <div className="modal-footer">
                      <Button  class="primarySmall" onclick={resendOtp} >Resend OTP</Button>
